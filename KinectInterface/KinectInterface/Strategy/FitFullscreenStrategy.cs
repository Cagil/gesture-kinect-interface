...
{
    class FitFullscreenStrategy : FittingStrategy
    {
        public BoundingRectangle fit(BoundingRectangle container, int width, int height)
        {
            ...
            float xScale = Math.Max(min, xRatio); // smaller than 1 means image-width is smaller than the container-width
            float yScale = Math.Max(min, yRatio); // smaller than 1 means image-height is smaller than the container-height
